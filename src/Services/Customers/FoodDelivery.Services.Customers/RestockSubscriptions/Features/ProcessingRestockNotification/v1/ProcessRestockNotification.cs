...
internal record ProcessRestockNotification(long ProductId, int CurrentStock) : ITransactionCommand
{
...
) : ICommandHandler<ProcessRestockNotification>
{
    public async Task Handle(ProcessRestockNotification command, CancellationToken cancellationToken)
    {
        ...
        var subscribedCustomers = customersDbContext.RestockSubscriptions
            .Where(x => x.ProductInformation.Id == command.ProductId 
                        && !x.Processed);
        ...
        foreach (var restockSubscription in subscribedCustomers)
        {
            ...
            // schedule `SendRestockNotification` for running as an internal command after commenting transaction
