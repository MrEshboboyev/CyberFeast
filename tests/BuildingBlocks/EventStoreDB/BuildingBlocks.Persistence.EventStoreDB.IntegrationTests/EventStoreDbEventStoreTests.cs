...
public class EventStoreDbEventStoreTests(IntegrationFixture integrationFixture) : IClassFixture<IntegrationFixture>
{
    [Fact]
    public async Task exist_stream_should_return_true_for_existing_stream()
    {
        ...
        var exists = await integrationFixture.EventStore.StreamExists(streamId);
    ...
    public async Task append_multiple_and_get_stream_events_with_version_should_return_correct_events()
    {
        ...
        var appendResult = await integrationFixture.EventStore.AppendEventsAsync(
            streamId,
            streamEvents,
        ...
        var events = (
            await integrationFixture.EventStore.GetStreamEventsAsync(
                streamId,
    ...
    public async Task append_single_to_existing_stream_and_get_stream_events_with_version_should_return_correct_events()
    {
        ...
        var aggregate = await integrationFixture.EventStore.AggregateStreamAsync<ShoppingCart, Guid>(
            streamId,
            StreamReadPosition.Start,
        ...
        var appendResult = await integrationFixture.EventStore.AppendEventAsync(
            streamId,
            streamEvent,
