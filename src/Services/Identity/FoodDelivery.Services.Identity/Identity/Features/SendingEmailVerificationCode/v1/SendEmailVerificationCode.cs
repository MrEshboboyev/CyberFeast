...
) : ICommandHandler<SendEmailVerificationCode>
{
    public async Task Handle(SendEmailVerificationCode command, CancellationToken cancellationToken)
    {
        ...
        var isExists = await context
            .Set<EmailVerificationCode>()
            .AnyAsync(evc => evc.Email == command.Email 
                             && evc.SentAt.AddMinutes(5) > DateTime.Now, 
                cancellationToken);
        ...
        var verificationCode = randomNumber.ToString("D6", CultureInfo.InvariantCulture);
        ...
        const string subject = "Verification Email";

        var emailObject = new EmailObject(command.Email, subject, content);
