...
{
    public static dynamic CreateGenericType(
        Type genericType,
        Type[] typeArguments,
    ...
    public static dynamic CreateGenericType<TGenericType>(
        Type[] typeArguments, 
        params object?[] constructorArgs)
    {
    ...
    public static string? GetPropertyName<T>(Expression<Func<T, object>> propertyExpression)
    {
        ...
        if (propertyExpression == null) return retVal;
    ...
    {
        var genericArguments = typeof(TResult)
            .GetTypeInfo()
            .GetGenericArguments();
        
        var methodArgumentList = methodInfo
            .GetParameters()
            .Select(p => p.ParameterType).ToList();
        
        var funcArgumentList = genericArguments
            .Skip(1)
            .Take(methodArgumentList.Count)
            .ToList();
        ...
        var argumentPairs = funcArgumentList
            .Zip(methodArgumentList, (s, d) => new
            {
        ...
        if (argumentPairs.All(a => a.Source == a.Destination))
        {
            ...
            var parameters = funcArgumentList
        ...
        var blockExpressions = new List<Expression>
        {
            Expression.Assign(instanceVariable,
                Expression.ConvertChecked(instanceArgument, type!))
        };
    ...
    {
        var rootNamespace = rootAssembly
            .GetName()
            .Name!
            .Split('.')
            .First();
        ...
        var list = rootAssembly!
            ...
            .Where(x => x.AssemblyName
                .StartsWith(rootNamespace, StringComparison.InvariantCulture))
    ...
    public static IReadOnlyList<Assembly> GetReferencedAssemblies(Assembly? rootAssembly)
    {
        ...
        while (queue.Count != 0)
        {
            ...
            {
                if (visited.Contains(reference.FullName)) continue;
