...
internal record GenerateRefreshToken(
    Guid UserId, 
    string? Token = null) : ICommand<GenerateRefreshTokenResult>
{
    public static GenerateRefreshToken Of(
        Guid userId, 
        string? token = null) => new(userId.NotBeInvalid(), token);
}
    ...
    : ICommandHandler<GenerateRefreshToken, GenerateRefreshTokenResult>
{
    ...
    )
    {
        ...
        var refreshToken = await context
            ...
            .FirstOrDefaultAsync(rt => rt.UserId == command.UserId 
                                       && rt.Token == command.Token, cancellationToken);
    ...
    private Task<int> RemoveOldRefreshTokens(
        Guid userId, 
