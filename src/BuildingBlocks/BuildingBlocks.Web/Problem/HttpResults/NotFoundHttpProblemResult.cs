...
/// <summary>
/// Represents an HTTP result for a 404 Not Found response with a problem details object.
/// </summary>
public class NotFoundHttpProblemResult : IResult, IStatusCodeHttpResult, IContentTypeHttpResult, IValueHttpResult,
    IEndpointMetadataProvider
{
    private readonly ProblemHttpResult _internalResult;

    /// <summary>
    /// Initializes a new instance of the <see cref="NotFoundHttpProblemResult"/> class.
    /// </summary>
    ...
    /// <exception cref="ArgumentException">Thrown if the status code is not 404 Not Found.</exception>
    internal NotFoundHttpProblemResult(ProblemDetails problemDetails)
    {
        ...
        if (problemDetails is { Status: not null and not StatusCodes.Status404NotFound })
        {
            throw new ArgumentException(
                $"{nameof(NotFoundHttpProblemResult)} only supports a 404 Not Found response status code.",
    ...
    public static void PopulateMetadata(MethodInfo method, EndpointBuilder builder)
    {
        ...
        builder.Metadata.Add(new ProducesResponseTypeAttribute(
