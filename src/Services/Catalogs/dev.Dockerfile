...
RUN --mount=type=cache,id=catalogs_nuget,target=/root/.nuget/packages \
  dotnet restore ./Services/Catalogs/FoodDelivery.Services.Catalogs.Api/FoodDelivery.Services.Catalogs.Api.csproj
